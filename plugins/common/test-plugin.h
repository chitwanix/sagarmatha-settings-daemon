...
has_settings (void)
{
	...
	for (i = 0; list[i] != NULL; i++) {
		if (g_str_equal (list[i], "org.sagarmatha.settings-daemon.plugins." SCHEMA_NAME))
			return TRUE;
	}
	return FALSE;
}
...
main (int argc, char **argv)
{
	...
	if (has_settings () == FALSE) {
        ...
        settings = g_settings_new ("org.sagarmatha.settings-daemon.plugins." SCHEMA_NAME);
		...
		fprintf (stderr, "\tgsettings set org.sagarmatha.settings-daemon.plugins." SCHEMA_NAME " active false\n");
		fprintf (stderr, "To reactivate:\n");
		fprintf (stderr, "\tgsettings set org.sagarmatha.settings-daemon.plugins." SCHEMA_NAME " active true\n");
		exit (1);
	}
