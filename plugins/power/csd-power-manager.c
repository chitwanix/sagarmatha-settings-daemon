...
#include "sagarmatha-settings-session.h"
#include "csd-enums.h"
...
#define CSD_POWER_SETTINGS_SCHEMA               "org.sagarmatha.settings-daemon.plugins.power"

#define CSD_DBUS_SERVICE                        "org.sagarmatha.SettingsDaemon"
#define CSD_DBUS_PATH                           "/org/sagarmatha/SettingsDaemon"
#define CSD_POWER_DBUS_PATH                     CSD_DBUS_PATH "/Power"
#define CSD_POWER_DBUS_INTERFACE                "org.sagarmatha.SettingsDaemon.Power"
#define CSD_POWER_DBUS_INTERFACE_SCREEN         "org.sagarmatha.SettingsDaemon.Power.Screen"
#define CSD_POWER_DBUS_INTERFACE_KEYBOARD       "org.sagarmatha.SettingsDaemon.Power.Keyboard"

#define GS_DBUS_NAME                            "org.sagarmatha.ScreenSaver"
#define GS_DBUS_PATH                            "/org/sagarmatha/ScreenSaver"
#define GS_DBUS_INTERFACE                       "org.sagarmatha.ScreenSaver"
...
"<node>"
  "<interface name='org.sagarmatha.SettingsDaemon.Power'>"
    ...
    "</method>"
  "</interface>"
"  <interface name='org.sagarmatha.SettingsDaemon.Power.Screen'>"
"    <method name='StepUp'>"
...
"    </signal>"
"  </interface>"
"  <interface name='org.sagarmatha.SettingsDaemon.Power.Keyboard'>"
"    <method name='StepUp'>"
"      <arg type='u' name='new_percentage' direction='out'/>"
"    </method>"
"    <method name='StepDown'>"
...
static void
sagarmatha_session_shutdown_cb (GObject *source_object,
                           GAsyncResult *res,
                           gpointer user_data)
{
        GVariant *result;
        ...
        result = g_dbus_proxy_call_finish (G_DBUS_PROXY (source_object),
                                           ...
                                           &error);
        if (result == NULL) {
                g_warning ("couldn't shutdown using sagarmatha-session: %s",
                           error->message);
                g_error_free (error);
        } else {
...
static void
sagarmatha_session_shutdown (void)
{
        GError *error = NULL;
        GDBusProxy *proxy;

        /* ask sagarmatha-session to show the shutdown dialog with a timeout */
        proxy = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION,
                                               ...
                                               NULL, &error);
        if (proxy == NULL) {
                g_warning ("cannot connect to sagarmatha-session: %s",
                           error->message);
                g_error_free (error);
                return;
        }
        g_dbus_proxy_call (proxy,
                           ...
                           -1, NULL,
                           sagarmatha_session_shutdown_cb, NULL);
...
idle_is_session_idle (CsdPowerManager *manager)
{
        ...
        /* not yet connected to sagarmatha-session */
        if (manager->priv->session_presence_proxy == NULL) {
                g_warning ("session idleness not available, sagarmatha-session is not available");
                return FALSE;
        }
...
idle_is_session_inhibited (CsdPowerManager *manager, guint mask)
{
        ...
        if (manager->priv->session_proxy == NULL) {
                g_warning ("session inhibition not available, sagarmatha-session is not available");
                return FALSE;
        }
                            ...
                            gpointer user_data)
{
        ...
        if (manager->priv->screensaver_proxy == NULL) {
                g_warning ("Could not connect to sagarmatha-screensaver: %s",
                           error->message);
                g_error_free (error);
                return;
        }
        ...
        g_debug ("sagarmatha-screensaver activated, doing sagarmatha-screensaver lock");
                        ...
                        gpointer user_data)
{
        ...
        if (manager->priv->session_proxy == NULL) {
                g_warning ("Could not connect to sagarmatha-session: %s",
                           error->message);
                g_error_free (error);
        } else {
...
static void
engine_session_active_changed_cb (SagarmathaSettingsSession *session,
                                  GParamSpec *pspec,
                                  CsdPowerManager *manager)
{
...
/* This timer goes off every few minutes, whether the user is idle or not,
   ...
   This code was originally part of sagarmatha-screensaver, see
   http://git.gnome.org/browse/sagarmatha-screensaver/tree/src/gs-watcher-x11.c?id=fec00b12ec46c86334cfd36b37771cc4632f0d4d#n530
 */
static gboolean
                         ...
                         GError **error)
{
        ...
        manager->priv->session = sagarmatha_settings_session_new ();
        ...
        manager->priv->settings_screensaver = g_settings_new ("org.sagarmatha.desktop.screensaver");
        ...
        if (g_strcmp0 (method_name, "GetPrimaryDevice") == 0) {
                ...
                if (device == NULL) {
                        g_dbus_method_invocation_return_dbus_error (invocation,
                                                                    "org.sagarmatha.SettingsDaemon.Power.Failed",
