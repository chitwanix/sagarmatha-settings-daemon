...
"<node>"
"  <interface name='org.sagarmatha.SettingsDaemon.KeybindingHandler'>"
"    <annotation name='org.freedesktop.DBus.GLib.CSymbol' value='csd_media_keys_manager'/>"
...
#define SETTINGS_INTERFACE_DIR "org.sagarmatha.desktop.interface"
#define SETTINGS_POWER_DIR "org.sagarmatha.settings-daemon.plugins.power"
#define SETTINGS_XSETTINGS_DIR "org.sagarmatha.settings-daemon.plugins.xsettings"
#define SETTINGS_TOUCHPAD_DIR "org.sagarmatha.settings-daemon.peripherals.touchpad"
...
static void
sagarmatha_session_shutdown (CsdMediaKeysManager *manager)
{
	GError *error = NULL;
	GVariant *variant;
	...
	if (manager->priv->connection == NULL) {
		execute (manager, "sagarmatha-session-quit --logout", FALSE);
		return;
	}
...
{    
        ...
        if (quiet == FALSE && sound_changed != FALSE && muted == FALSE) {                            
                GSettings *settings = g_settings_new ("org.sagarmatha.desktop.sound");
...
static void
csd_media_keys_manager_handle_sagarmatha_keybinding (CsdMediaKeysManager *manager,
                                                   guint                deviceid,
                                                   MediaKeyType         type,
                                                   gint64               timestamp)
{
                    ...
                    gpointer               user_data)
{
        ...
        if (g_strcmp0 (method_name, "ReleaseMediaPlayerKeys") == 0) {
                ...
                csd_media_keys_manager_handle_sagarmatha_keybinding (manager, NULL, action, CurrentTime);
                g_dbus_method_invocation_return_value (invocation, NULL);
        }
}
...
do_toggle_accessibility_key (const char *key)
{
        ...
        settings = g_settings_new ("org.sagarmatha.desktop.a11y.applications");
			  ...
			  MediaKeyType         type)
{
	...
	settings = g_settings_new ("org.sagarmatha.desktop.a11y.magnifier");
                        ...
                        const gchar *config_key)
{
        ...
        case CSD_POWER_ACTION_INTERACTIVE:
                sagarmatha_session_shutdown (manager);
                break;
        case CSD_POWER_ACTION_SHUTDOWN:
                execute (manager, "sagarmatha-session-quit --power-off --no-prompt", FALSE);
                break;
        case CSD_POWER_ACTION_HIBERNATE:
                csd_power_hibernate (manager->priv->upower_proxy);
                break;
        case CSD_POWER_ACTION_BLANK:
                execute (manager, "sagarmatha-screensaver-command --lock", FALSE);
                break;
...
start_media_keys_idle_cb (CsdMediaKeysManager *manager)
{
	...
	manager->priv->icon_theme = g_settings_get_string (manager->priv->interface_settings, "icon-theme");
        ...
        for (l = manager->priv->screens; l != NULL; l = l->next) {
                sagarmatha_settings_profile_start ("gdk_window_add_filter");
                ...
                gdk_window_add_filter (gdk_screen_get_root_window (l->data),
                                       ...
                                       manager);
                sagarmatha_settings_profile_end ("gdk_window_add_filter");
        }

        GSettings *settings = g_settings_new ("org.sagarmatha.sounds");
                              ...
                              GError             **error)
{
         ...
         * idle.
         */
        sagarmatha_settings_profile_start ("gvc_mixer_control_new");

        manager->priv->volume = gvc_mixer_control_new ("Sagarmatha Volume Control Media Keys");
        ...
        sagarmatha_settings_profile_end ("gvc_mixer_control_new");
               ...
               CsdMediaKeysManager *manager)
{
        ...
        g_dbus_proxy_new (manager->priv->connection,
                          ...
                          "org.sagarmatha.SettingsDaemon",
                          "/org/sagarmatha/SettingsDaemon/Power",
                          "org.sagarmatha.SettingsDaemon.Power.Keyboard",
                          NULL,
