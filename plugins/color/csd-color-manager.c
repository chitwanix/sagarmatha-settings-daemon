...
struct CsdColorManagerPrivate
{
        ...
        SagarmathaSettingsSessionState session_state;
...
static guint
sagarmatha_rr_output_get_gamma_size (GnomeRROutput *output)
{
        GnomeRRCrtc *crtc;
        gint len = 0;
                              ...
                              GError **error)
{
        ...
        size = sagarmatha_rr_output_get_gamma_size (output);
        if (size == 0) {
                g_set_error_literal (error,
                                     CSD_COLOR_MANAGER_ERROR,
                                     CSD_COLOR_MANAGER_ERROR_FAILED,
...
#define CSD_DBUS_INTERFACE_POWER_SCREEN	"org.sagarmatha.SettingsDaemon.Power.Screen"
#define CSD_DBUS_PATH_POWER		"/org/sagarmatha/SettingsDaemon/Power"
...
static void
sagarmatha_rr_screen_output_added_cb (GnomeRRScreen *screen,
                                GnomeRROutput *output,
                                CsdColorManager *manager)
{
...
static void
sagarmatha_rr_screen_output_removed_cb (GnomeRRScreen *screen,
                                   GnomeRROutput *output,
                                   CsdColorManager *manager)
{
...
static void
sagarmatha_rr_screen_output_changed_cb (GnomeRRScreen *screen,
                                   CsdColorManager *manager)
{
        GnomeRROutput **outputs;
        CsdColorManagerPrivate *priv = manager->priv;
                               ...
                               gpointer user_data)
{
        ...
        g_signal_connect (priv->x11_screen, "output-connected",
                          G_CALLBACK (sagarmatha_rr_screen_output_added_cb),
                          manager);
        g_signal_connect (priv->x11_screen, "output-disconnected",
                          G_CALLBACK (sagarmatha_rr_screen_output_removed_cb),
                          manager);
        g_signal_connect (priv->x11_screen, "changed",
                          G_CALLBACK (sagarmatha_rr_screen_output_changed_cb),
                          manager);
...
gcm_session_exec_control_center (CsdColorManager *manager)
{
        ...
        app_info = g_app_info_create_from_commandline (BINDIR "/sagarmatha-settings color",
                                                       "sagarmatha-settings",
        ...
        /* launch sagarmatha-settings */
        ret = g_app_info_launch (app_info,
                                 NULL,
                                 G_APP_LAUNCH_CONTEXT (launch_context),
                                 &error);
        if (!ret) {
                g_warning ("failed to launch sagarmatha-settings: %s",
                           error->message);
                g_error_free (error);
                goto out;
        }
                             ...
                             CsdColorManager *manager)
{
        ca_context_play (ca_gtk_context_get (), 0,
                         ...
                         CA_PROP_APPLICATION_NAME, _("Sagarmatha Settings Daemon Color Plugin"),
...
static void
gcm_session_active_changed_cb (SagarmathaSettingsSession *session,
                               GParamSpec *pspec,
                               CsdColorManager *manager)
{
        SagarmathaSettingsSessionState state_new;
        CsdColorManagerPrivate *priv = manager->priv;
         ...
         *
         * If this is the first time the SagarmathaSettingsSession has been
...
csd_color_manager_init (CsdColorManager *manager)
{
        ...
        priv->session = sagarmatha_settings_session_new ();
        priv->session_state = sagarmatha_settings_session_get_state (priv->session);
        ...
        priv->settings = g_settings_new ("org.sagarmatha.settings-daemon.plugins.color");
