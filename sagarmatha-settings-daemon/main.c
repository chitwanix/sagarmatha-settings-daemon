...
static SagarmathaSettingsManager *manager = NULL;
...
static void
stop_manager (SagarmathaSettingsManager *manager)
{
...
set_locale (GDBusProxy *proxy)
{
        ...
        locale_settings = g_settings_new ("org.sagarmatha.system.locale");
...
register_with_gnome_session (GDBusProxy *proxy)
{
        ...
        g_dbus_proxy_call (proxy,
                           "RegisterClient",
                           g_variant_new ("(ss)", "sagarmatha-settings-daemon", startup_id ? startup_id : ""),
                           G_DBUS_CALL_FLAGS_NONE,
                           -1,
                           NULL,
...
keyboard_plugin_is_enabled (void)
{
        ...
        settings = g_settings_new ("org.sagarmatha.settings-daemon.plugins.keyboard");
...
static void
watch_for_term_signal (SagarmathaSettingsManager *manager)
{
        GIOChannel *channel;
...
queue_register_client (void)
{
        ...
        /* Register the daemon with sagarmatha-session */
        g_signal_connect (G_OBJECT (proxy), "g-signal",
                          G_CALLBACK (on_session_over), NULL);
...
main (int argc, char *argv[])
{
        ...
        sagarmatha_settings_profile_start ("opening gtk display");
        if (! gtk_init_check (NULL, NULL)) {
                g_warning ("Unable to initialize GTK+");
                exit (EXIT_FAILURE);
        }
        sagarmatha_settings_profile_end ("opening gtk display");
        ...
        sagarmatha_settings_profile_start ("sagarmatha_settings_manager_new");
        manager = sagarmatha_settings_manager_new ();
        sagarmatha_settings_profile_end ("sagarmatha_settings_manager_new");
        if (manager == NULL) {
        ...
        error = NULL;
        res = sagarmatha_settings_manager_start (manager, &error);
        if (! res) {
