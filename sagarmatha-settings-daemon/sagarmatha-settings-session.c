...
struct SagarmathaSettingsSessionPrivate
{
#ifdef HAVE_SYSTEMD
        GSource                   *sd_source;
#else
...
G_DEFINE_TYPE (SagarmathaSettingsSession, sagarmatha_settings_session, G_TYPE_OBJECT)
...
static void
sagarmatha_settings_session_set_state (SagarmathaSettingsSession *session,
				  gboolean active)
{
        SagarmathaSettingsSessionState  state;
...
static void
sagarmatha_settings_session_get_property (GObject *object,
				     guint prop_id,
				     GValue *value,
				     GParamSpec *pspec)
{
...
GType
sagarmatha_settings_session_state_get_type (void)
{
	...
	if (etype == 0) {
		...
		etype = g_enum_register_static ("SagarmathaSettingsSessionState", values);
	}
	return etype;
}

static void
sagarmatha_settings_session_class_init (SagarmathaSettingsSessionClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	object_class->get_property = sagarmatha_settings_session_get_property;
	object_class->finalize = sagarmatha_settings_session_finalize;
	g_type_class_add_private (klass, sizeof (SagarmathaSettingsSessionPrivate));
...
{
        SagarmathaSettingsSession *session = user_data;
        gboolean active;

        active = sd_session_is_active (session->priv->session_id);
        sagarmatha_settings_session_set_state (session, active);
...
static void
sagarmatha_settings_session_proxy_signal_cb (GDBusProxy *proxy,
...
is_active_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
	...
	g_signal_connect (session->priv->proxy_session, "g-signal",
			  G_CALLBACK (sagarmatha_settings_session_proxy_signal_cb),
			  session);
...
static void
sagarmatha_settings_session_finalize (GObject *object)
{
	SagarmathaSettingsSession *session;
	...
	G_OBJECT_CLASS (sagarmatha_settings_session_parent_class)->finalize (object);
}
